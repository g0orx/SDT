...
//  int selectedMapIndex;
  ...
  DrawKeyboard();
  CaptureKeystrokes();
  retVal = BearingHeading(keyboardBuffer);  // If 0.0
  if (retVal == -1.0) {                     // Didn't find country
    RedrawDisplayScreen();
    ShowFrequency();
  ...
  if (retVal != -0.0) {        // We have valid country
    if (sdCardPresent != 0) {  // We have a map
      bmpDraw((char *)myMapFiles[selectedMapIndex].mapNames, IMAGE_CORNER_X, IMAGE_CORNER_Y);
      doneViewing = false;
    } else {
      tft.fillRect(200, 300, tft.getFontWidth() * 12, tft.getFontHeight(), RA8875_BLACK);  // No map
      tft.drawCircle(400, 240, 150, RA8875_WHITE);
      tft.setTextColor(RA8875_WHITE, RA8875_BLACK);
      homeLat = myMapFiles[selectedMapIndex].lat;
      homeLon = myMapFiles[selectedMapIndex].lon;  // your QTH longitude
      ...
      tft.print(bearingDistance);
      tft.print(" km");
      tft.setCursor(155, 70);
      tft.print(displayBearing);
      tft.setTextColor(RA8875_WHITE, RA8875_BLACK);
      VectorCalc();
      MyDelay(1000L);
    }
  } else {
    return -1;  // Cpuldn't find country
  }
  while (true) {
    valPin = ReadSelectedPushButton();  // Poll UI push buttons
    MyDelay(100L);
...
int VectorCalc() {
  ...
  for (int i = -1; i < 1; i++) {
    tft.drawLineAngle(400, 240, displayBearing + (float)i * 1.0, 150, RA8875_RED, -90);  // RAY_LENGTH
  }
  return 1;
}